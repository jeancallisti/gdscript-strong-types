#-----------------------------------------------------------------
#
#           THIS FILE WAS AUTOGENERATED.
#
#           Visit https://github.com/jeancallisti/gdscript-strong-types
#
#
#-----------------------------------------------------------------


# This class is a utility class meant to represent an array of PacketPeerUDP.
# it is not meant to be attached to a Node (unless you want to!)
# It exists in two versions (both named 'PacketPeerUDPArray'): The one in CSharp style (CapitalLetters) or the one in GDScript style (snake_case).
# Just uncomment the version you like the most.

class_name PacketPeerUDPArray

var a:Array = [];

func _crash(message: String):
    assert(false, message)

func _assert_PacketPeerUDP(value: PacketPeerUDP):
    if (!value is PacketPeerUDP):
        _crash("value was expected to be of type PacketPeerUDP")

func _assert_not_null_PacketPeerUDP(value: PacketPeerUDP):
    if (value == null):
        _crash("value was expected to be not null")
    _assert_PacketPeerUDP(value)

func SetAt(position: int, value: PacketPeerUDP):
    _assert_PacketPeerUDP(value);
    a[position] = value;

func GetAt(position: int) -> PacketPeerUDP:
    return a[position];

func Add(value: PacketPeerUDP):
    _assert_PacketPeerUDP(value);
    a.append(value);

# Same as Add, but with a different name.
func Append(value: PacketPeerUDP):
    _assert_PacketPeerUDP(value);
    a.append(value);

func Remove(position: int):
    a.remove(position);

func Size() -> int:
    return a.size();

func Clear():
    a.clear();

# func Contains(value: PacketPeerUDP) -> bool:
#     _assert_PacketPeerUDP(value);
#     return a.contains(value);

