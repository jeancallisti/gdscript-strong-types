#-----------------------------------------------------------------
#
#           THIS FILE WAS AUTOGENERATED.
#
#           Visit https://github.com/jeancallisti/gdscript-strong-types
#
#
#-----------------------------------------------------------------


# This class is a utility class meant to represent an array of InputEventPanGesture.
# it is not meant to be attached to a Node (unless you want to!)
# It exists in two versions (both named 'InputEventPanGestureArray'): The one in CSharp style (CapitalLetters) or the one in GDScript style (snake_case).
# Just uncomment the version you like the most.

class_name InputEventPanGestureArray

var a:Array = [];

func _crash(message: String):
    assert(false, message)

func _assert_InputEventPanGesture(value: InputEventPanGesture):
    if (!value is InputEventPanGesture):
        _crash("value was expected to be of type InputEventPanGesture")

func _assert_not_null_InputEventPanGesture(value: InputEventPanGesture):
    if (value == null):
        _crash("value was expected to be not null")
    _assert_InputEventPanGesture(value)

func SetAt(position: int, value: InputEventPanGesture):
    _assert_InputEventPanGesture(value);
    a[position] = value;

func GetAt(position: int) -> InputEventPanGesture:
    return a[position];

func Add(value: InputEventPanGesture):
    _assert_InputEventPanGesture(value);
    a.append(value);

# Same as Add, but with a different name.
func Append(value: InputEventPanGesture):
    _assert_InputEventPanGesture(value);
    a.append(value);

func Remove(position: int):
    a.remove(position);

func Size() -> int:
    return a.size();

func Clear():
    a.clear();

# func Contains(value: InputEventPanGesture) -> bool:
#     _assert_InputEventPanGesture(value);
#     return a.contains(value);

