...
# where keys are of type 'String' and values of type 'Joint'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name JointDictionary
...
func _assert_not_null_Joint(value: Joint):
    if (!value is Joint):
        _crash("Was expecting type 'Joint'")
