...
# where keys are of type 'String' and values of type 'SpinBox'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpinBoxDictionary
...
func _assert_not_null_SpinBox(value: SpinBox):
    if (!value is SpinBox):
        _crash("Was expecting type 'SpinBox'")
