...
# where keys are of type 'String' and values of type 'Shape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ShapeDictionary
...
func _assert_not_null_Shape(value: Shape):
    if (!value is Shape):
        _crash("Was expecting type 'Shape'")
