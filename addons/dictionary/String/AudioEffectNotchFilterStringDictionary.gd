...
# where keys are of type 'String' and values of type 'AudioEffectNotchFilter'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioEffectNotchFilterDictionary
...
func _assert_not_null_AudioEffectNotchFilter(value: AudioEffectNotchFilter):
    if (!value is AudioEffectNotchFilter):
        _crash("Was expecting type 'AudioEffectNotchFilter'")
