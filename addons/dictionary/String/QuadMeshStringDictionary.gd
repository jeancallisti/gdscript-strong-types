...
# where keys are of type 'String' and values of type 'QuadMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name QuadMeshDictionary
...
func _assert_not_null_QuadMesh(value: QuadMesh):
    if (!value is QuadMesh):
        _crash("Was expecting type 'QuadMesh'")
