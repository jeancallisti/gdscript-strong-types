...
# where keys are of type 'String' and values of type 'BoxShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name BoxShapeDictionary
...
func _assert_not_null_BoxShape(value: BoxShape):
    if (!value is BoxShape):
        _crash("Was expecting type 'BoxShape'")
