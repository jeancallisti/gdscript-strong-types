...
# where keys are of type 'String' and values of type 'Range'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RangeDictionary
...
func _assert_not_null_Range(value: Range):
    if (!value is Range):
        _crash("Was expecting type 'Range'")
