...
# where keys are of type 'String' and values of type 'AcceptDialog'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AcceptDialogDictionary
...
func _assert_not_null_AcceptDialog(value: AcceptDialog):
    if (!value is AcceptDialog):
        _crash("Was expecting type 'AcceptDialog'")
