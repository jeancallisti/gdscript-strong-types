...
# where keys are of type 'String' and values of type 'Transform'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TransformDictionary
...
func _assert_not_null_Transform(value: Transform):
    if (!value is Transform):
        _crash("Was expecting type 'Transform'")
