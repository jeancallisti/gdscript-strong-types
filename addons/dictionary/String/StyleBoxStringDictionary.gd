...
# where keys are of type 'String' and values of type 'StyleBox'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StyleBoxDictionary
...
func _assert_not_null_StyleBox(value: StyleBox):
    if (!value is StyleBox):
        _crash("Was expecting type 'StyleBox'")
