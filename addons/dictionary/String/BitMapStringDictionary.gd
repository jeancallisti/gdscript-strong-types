...
# where keys are of type 'String' and values of type 'BitMap'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name BitMapDictionary
...
func _assert_not_null_BitMap(value: BitMap):
    if (!value is BitMap):
        _crash("Was expecting type 'BitMap'")
