...
# where keys are of type 'String' and values of type 'OmniLight'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name OmniLightDictionary
...
func _assert_not_null_OmniLight(value: OmniLight):
    if (!value is OmniLight):
        _crash("Was expecting type 'OmniLight'")
