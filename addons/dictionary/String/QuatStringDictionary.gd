...
# where keys are of type 'String' and values of type 'Quat'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name QuatDictionary
...
func _assert_not_null_Quat(value: Quat):
    if (!value is Quat):
        _crash("Was expecting type 'Quat'")
