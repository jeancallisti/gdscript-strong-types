...
# where keys are of type 'String' and values of type 'Navigation'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NavigationDictionary
...
func _assert_not_null_Navigation(value: Navigation):
    if (!value is Navigation):
        _crash("Was expecting type 'Navigation'")
