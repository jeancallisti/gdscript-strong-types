...
# where keys are of type 'String' and values of type 'Path2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Path2DDictionary
...
func _assert_not_null_Path2D(value: Path2D):
    if (!value is Path2D):
        _crash("Was expecting type 'Path2D'")
