...
# where keys are of type 'String' and values of type 'CylinderMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CylinderMeshDictionary
...
func _assert_not_null_CylinderMesh(value: CylinderMesh):
    if (!value is CylinderMesh):
        _crash("Was expecting type 'CylinderMesh'")
