...
# where keys are of type 'String' and values of type 'Curve3D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Curve3DDictionary
...
func _assert_not_null_Curve3D(value: Curve3D):
    if (!value is Curve3D):
        _crash("Was expecting type 'Curve3D'")
