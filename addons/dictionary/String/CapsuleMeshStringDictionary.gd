...
# where keys are of type 'String' and values of type 'CapsuleMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CapsuleMeshDictionary
...
func _assert_not_null_CapsuleMesh(value: CapsuleMesh):
    if (!value is CapsuleMesh):
        _crash("Was expecting type 'CapsuleMesh'")
