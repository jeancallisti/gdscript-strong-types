...
# where keys are of type 'String' and values of type 'HBoxContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HBoxContainerDictionary
...
func _assert_not_null_HBoxContainer(value: HBoxContainer):
    if (!value is HBoxContainer):
        _crash("Was expecting type 'HBoxContainer'")
