...
# where keys are of type 'String' and values of type 'HScrollBar'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HScrollBarDictionary
...
func _assert_not_null_HScrollBar(value: HScrollBar):
    if (!value is HScrollBar):
        _crash("Was expecting type 'HScrollBar'")
