...
# where keys are of type 'String' and values of type 'Viewport'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ViewportDictionary
...
func _assert_not_null_Viewport(value: Viewport):
    if (!value is Viewport):
        _crash("Was expecting type 'Viewport'")
