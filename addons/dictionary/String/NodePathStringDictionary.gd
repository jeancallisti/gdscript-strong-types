...
# where keys are of type 'String' and values of type 'NodePath'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NodePathDictionary
...
func _assert_not_null_NodePath(value: NodePath):
    if (!value is NodePath):
        _crash("Was expecting type 'NodePath'")
