...
# where keys are of type 'String' and values of type 'SpriteBase3D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpriteBase3DDictionary
...
func _assert_not_null_SpriteBase3D(value: SpriteBase3D):
    if (!value is SpriteBase3D):
        _crash("Was expecting type 'SpriteBase3D'")
