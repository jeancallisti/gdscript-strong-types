...
# where keys are of type 'String' and values of type 'CPUParticles2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CPUParticles2DDictionary
...
func _assert_not_null_CPUParticles2D(value: CPUParticles2D):
    if (!value is CPUParticles2D):
        _crash("Was expecting type 'CPUParticles2D'")
