...
# where keys are of type 'String' and values of type 'Shape2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Shape2DDictionary
...
func _assert_not_null_Shape2D(value: Shape2D):
    if (!value is Shape2D):
        _crash("Was expecting type 'Shape2D'")
