...
# where keys are of type 'String' and values of type 'ProjectSettings'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ProjectSettingsDictionary
...
func _assert_not_null_ProjectSettings(value: ProjectSettings):
    if (!value is ProjectSettings):
        _crash("Was expecting type 'ProjectSettings'")
