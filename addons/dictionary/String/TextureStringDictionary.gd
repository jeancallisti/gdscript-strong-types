...
# where keys are of type 'String' and values of type 'Texture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TextureDictionary
...
func _assert_not_null_Texture(value: Texture):
    if (!value is Texture):
        _crash("Was expecting type 'Texture'")
