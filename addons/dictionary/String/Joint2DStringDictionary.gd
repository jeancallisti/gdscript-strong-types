...
# where keys are of type 'String' and values of type 'Joint2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Joint2DDictionary
...
func _assert_not_null_Joint2D(value: Joint2D):
    if (!value is Joint2D):
        _crash("Was expecting type 'Joint2D'")
