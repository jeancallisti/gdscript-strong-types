...
# where keys are of type 'String' and values of type 'InterpolatedCamera'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name InterpolatedCameraDictionary
...
func _assert_not_null_InterpolatedCamera(value: InterpolatedCamera):
    if (!value is InterpolatedCamera):
        _crash("Was expecting type 'InterpolatedCamera'")
