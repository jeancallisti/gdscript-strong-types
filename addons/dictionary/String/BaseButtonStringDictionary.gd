...
# where keys are of type 'String' and values of type 'BaseButton'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name BaseButtonDictionary
...
func _assert_not_null_BaseButton(value: BaseButton):
    if (!value is BaseButton):
        _crash("Was expecting type 'BaseButton'")
