...
# where keys are of type 'String' and values of type 'RigidBody2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RigidBody2DDictionary
...
func _assert_not_null_RigidBody2D(value: RigidBody2D):
    if (!value is RigidBody2D):
        _crash("Was expecting type 'RigidBody2D'")
