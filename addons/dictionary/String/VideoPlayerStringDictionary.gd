...
# where keys are of type 'String' and values of type 'VideoPlayer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name VideoPlayerDictionary
...
func _assert_not_null_VideoPlayer(value: VideoPlayer):
    if (!value is VideoPlayer):
        _crash("Was expecting type 'VideoPlayer'")
