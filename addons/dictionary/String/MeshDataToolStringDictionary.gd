...
# where keys are of type 'String' and values of type 'MeshDataTool'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MeshDataToolDictionary
...
func _assert_not_null_MeshDataTool(value: MeshDataTool):
    if (!value is MeshDataTool):
        _crash("Was expecting type 'MeshDataTool'")
