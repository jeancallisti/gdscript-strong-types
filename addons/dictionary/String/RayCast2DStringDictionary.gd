...
# where keys are of type 'String' and values of type 'RayCast2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RayCast2DDictionary
...
func _assert_not_null_RayCast2D(value: RayCast2D):
    if (!value is RayCast2D):
        _crash("Was expecting type 'RayCast2D'")
