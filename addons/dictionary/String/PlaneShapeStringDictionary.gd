...
# where keys are of type 'String' and values of type 'PlaneShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PlaneShapeDictionary
...
func _assert_not_null_PlaneShape(value: PlaneShape):
    if (!value is PlaneShape):
        _crash("Was expecting type 'PlaneShape'")
