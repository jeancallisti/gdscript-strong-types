...
# where keys are of type 'String' and values of type 'Line2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Line2DDictionary
...
func _assert_not_null_Line2D(value: Line2D):
    if (!value is Line2D):
        _crash("Was expecting type 'Line2D'")
