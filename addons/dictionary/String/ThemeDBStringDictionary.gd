...
# where keys are of type 'String' and values of type 'ThemeDB'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ThemeDBDictionary
...
func _assert_not_null_ThemeDB(value: ThemeDB):
    if (!value is ThemeDB):
        _crash("Was expecting type 'ThemeDB'")
