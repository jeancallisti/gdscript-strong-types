...
# where keys are of type 'String' and values of type 'GDScript'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GDScriptDictionary
...
func _assert_not_null_GDScript(value: GDScript):
    if (!value is GDScript):
        _crash("Was expecting type 'GDScript'")
