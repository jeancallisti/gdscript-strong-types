...
# where keys are of type 'String' and values of type 'AudioBusLayout'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioBusLayoutDictionary
...
func _assert_not_null_AudioBusLayout(value: AudioBusLayout):
    if (!value is AudioBusLayout):
        _crash("Was expecting type 'AudioBusLayout'")
