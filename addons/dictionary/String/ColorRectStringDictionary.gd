...
# where keys are of type 'String' and values of type 'ColorRect'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ColorRectDictionary
...
func _assert_not_null_ColorRect(value: ColorRect):
    if (!value is ColorRect):
        _crash("Was expecting type 'ColorRect'")
