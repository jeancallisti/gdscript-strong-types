...
# where keys are of type 'String' and values of type 'FuncRef'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name FuncRefDictionary
...
func _assert_not_null_FuncRef(value: FuncRef):
    if (!value is FuncRef):
        _crash("Was expecting type 'FuncRef'")
