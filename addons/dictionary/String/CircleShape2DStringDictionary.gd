...
# where keys are of type 'String' and values of type 'CircleShape2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CircleShape2DDictionary
...
func _assert_not_null_CircleShape2D(value: CircleShape2D):
    if (!value is CircleShape2D):
        _crash("Was expecting type 'CircleShape2D'")
