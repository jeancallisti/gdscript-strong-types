...
# where keys are of type 'String' and values of type 'Font'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name FontDictionary
...
func _assert_not_null_Font(value: Font):
    if (!value is Font):
        _crash("Was expecting type 'Font'")
