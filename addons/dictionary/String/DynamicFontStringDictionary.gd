...
# where keys are of type 'String' and values of type 'DynamicFont'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name DynamicFontDictionary
...
func _assert_not_null_DynamicFont(value: DynamicFont):
    if (!value is DynamicFont):
        _crash("Was expecting type 'DynamicFont'")
