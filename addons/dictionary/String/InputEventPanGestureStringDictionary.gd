...
# where keys are of type 'String' and values of type 'InputEventPanGesture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name InputEventPanGestureDictionary
...
func set(key: String, value: InputEventPanGesture):
...
func set_nullable(key: String, value: InputEventPanGesture):
    _assert_not_null(key)
    _assert_String(key)
    _assert_InputEventPanGesture(value)
    d[key] = value

func get(key: String) -> InputEventPanGesture:
    _assert_not_null(key)
    _assert_String(key)
    return d[key]	
