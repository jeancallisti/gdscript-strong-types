...
# where keys are of type 'String' and values of type 'RegEx'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RegExDictionary
...
func _assert_not_null_RegEx(value: RegEx):
    if (!value is RegEx):
        _crash("Was expecting type 'RegEx'")
