...
# where keys are of type 'String' and values of type 'ProgressBar'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ProgressBarDictionary
...
func _assert_not_null_ProgressBar(value: ProgressBar):
    if (!value is ProgressBar):
        _crash("Was expecting type 'ProgressBar'")
