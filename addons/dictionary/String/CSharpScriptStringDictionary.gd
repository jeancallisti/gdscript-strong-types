...
# where keys are of type 'String' and values of type 'CSharpScript'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSharpScriptDictionary
...
func _assert_not_null_CSharpScript(value: CSharpScript):
    if (!value is CSharpScript):
        _crash("Was expecting type 'CSharpScript'")
