...
# where keys are of type 'String' and values of type 'GridContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GridContainerDictionary
...
func _assert_not_null_GridContainer(value: GridContainer):
    if (!value is GridContainer):
        _crash("Was expecting type 'GridContainer'")
