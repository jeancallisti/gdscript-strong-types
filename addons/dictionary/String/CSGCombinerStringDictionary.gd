...
# where keys are of type 'String' and values of type 'CSGCombiner'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGCombinerDictionary
...
func _assert_not_null_CSGCombiner(value: CSGCombiner):
    if (!value is CSGCombiner):
        _crash("Was expecting type 'CSGCombiner'")
