...
# where keys are of type 'String' and values of type 'NativeScript'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NativeScriptDictionary
...
func _assert_not_null_NativeScript(value: NativeScript):
    if (!value is NativeScript):
        _crash("Was expecting type 'NativeScript'")
