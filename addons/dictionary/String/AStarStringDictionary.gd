...
# where keys are of type 'String' and values of type 'AStar'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AStarDictionary
...
func _assert_not_null_AStar(value: AStar):
    if (!value is AStar):
        _crash("Was expecting type 'AStar'")
