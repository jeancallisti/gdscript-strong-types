...
# where keys are of type 'String' and values of type 'VideoStream'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name VideoStreamDictionary
...
func _assert_not_null_VideoStream(value: VideoStream):
    if (!value is VideoStream):
        _crash("Was expecting type 'VideoStream'")
