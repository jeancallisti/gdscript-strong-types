...
# where keys are of type 'String' and values of type 'GLTFNode'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFNodeDictionary
...
func _assert_not_null_GLTFNode(value: GLTFNode):
    if (!value is GLTFNode):
        _crash("Was expecting type 'GLTFNode'")
