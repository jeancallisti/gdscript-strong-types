...
# where keys are of type 'String' and values of type 'HeightMapShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HeightMapShapeDictionary
...
func _assert_not_null_HeightMapShape(value: HeightMapShape):
    if (!value is HeightMapShape):
        _crash("Was expecting type 'HeightMapShape'")
