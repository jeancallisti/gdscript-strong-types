...
# where keys are of type 'String' and values of type 'SplitContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SplitContainerDictionary
...
func _assert_not_null_SplitContainer(value: SplitContainer):
    if (!value is SplitContainer):
        _crash("Was expecting type 'SplitContainer'")
