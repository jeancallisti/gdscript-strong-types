...
# where keys are of type 'String' and values of type 'SpringArm'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpringArmDictionary
...
func _assert_not_null_SpringArm(value: SpringArm):
    if (!value is SpringArm):
        _crash("Was expecting type 'SpringArm'")
