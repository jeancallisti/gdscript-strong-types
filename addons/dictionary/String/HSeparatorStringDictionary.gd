...
# where keys are of type 'String' and values of type 'HSeparator'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HSeparatorDictionary
...
func _assert_not_null_HSeparator(value: HSeparator):
    if (!value is HSeparator):
        _crash("Was expecting type 'HSeparator'")
