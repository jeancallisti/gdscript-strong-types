...
# where keys are of type 'String' and values of type 'CheckButton'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CheckButtonDictionary
...
func _assert_not_null_CheckButton(value: CheckButton):
    if (!value is CheckButton):
        _crash("Was expecting type 'CheckButton'")
