...
# where keys are of type 'String' and values of type 'Tree'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TreeDictionary
...
func _assert_not_null_Tree(value: Tree):
    if (!value is Tree):
        _crash("Was expecting type 'Tree'")
