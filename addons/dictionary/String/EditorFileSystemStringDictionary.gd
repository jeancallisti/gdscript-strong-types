...
# where keys are of type 'String' and values of type 'EditorFileSystem'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name EditorFileSystemDictionary
...
func _assert_not_null_EditorFileSystem(value: EditorFileSystem):
    if (!value is EditorFileSystem):
        _crash("Was expecting type 'EditorFileSystem'")
