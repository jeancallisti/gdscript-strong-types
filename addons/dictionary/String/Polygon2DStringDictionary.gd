...
# where keys are of type 'String' and values of type 'Polygon2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Polygon2DDictionary
...
func _assert_not_null_Polygon2D(value: Polygon2D):
    if (!value is Polygon2D):
        _crash("Was expecting type 'Polygon2D'")
