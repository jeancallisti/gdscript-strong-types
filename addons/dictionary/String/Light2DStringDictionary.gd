...
# where keys are of type 'String' and values of type 'Light2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Light2DDictionary
...
func _assert_not_null_Light2D(value: Light2D):
    if (!value is Light2D):
        _crash("Was expecting type 'Light2D'")
