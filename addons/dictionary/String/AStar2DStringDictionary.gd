...
# where keys are of type 'String' and values of type 'AStar2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AStar2DDictionary
...
func _assert_not_null_AStar2D(value: AStar2D):
    if (!value is AStar2D):
        _crash("Was expecting type 'AStar2D'")
