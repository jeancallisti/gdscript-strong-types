...
# where keys are of type 'String' and values of type 'PhysicsServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PhysicsServerDictionary
...
func _assert_not_null_PhysicsServer(value: PhysicsServer):
    if (!value is PhysicsServer):
        _crash("Was expecting type 'PhysicsServer'")
