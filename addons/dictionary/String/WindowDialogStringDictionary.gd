...
# where keys are of type 'String' and values of type 'WindowDialog'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name WindowDialogDictionary
...
func _assert_not_null_WindowDialog(value: WindowDialog):
    if (!value is WindowDialog):
        _crash("Was expecting type 'WindowDialog'")
