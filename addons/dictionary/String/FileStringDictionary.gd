...
# where keys are of type 'String' and values of type 'File'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name FileDictionary
...
func _assert_not_null_File(value: File):
    if (!value is File):
        _crash("Was expecting type 'File'")
