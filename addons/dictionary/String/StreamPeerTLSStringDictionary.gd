...
# where keys are of type 'String' and values of type 'StreamPeerTLS'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StreamPeerTLSDictionary
...
func set(key: String, value: StreamPeerTLS):
...
func set_nullable(key: String, value: StreamPeerTLS):
    _assert_not_null(key)
    _assert_String(key)
    _assert_StreamPeerTLS(value)
    d[key] = value

func get(key: String) -> StreamPeerTLS:
    _assert_not_null(key)
    _assert_String(key)
    return d[key]	
