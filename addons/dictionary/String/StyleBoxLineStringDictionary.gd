...
# where keys are of type 'String' and values of type 'StyleBoxLine'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StyleBoxLineDictionary
...
func _assert_not_null_StyleBoxLine(value: StyleBoxLine):
    if (!value is StyleBoxLine):
        _crash("Was expecting type 'StyleBoxLine'")
