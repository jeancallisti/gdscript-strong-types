...
# where keys are of type 'String' and values of type 'Path'.
...
func _assert_not_null_Path(value: Path):
    if (!value is Path):
        _crash("Was expecting type 'Path'")
