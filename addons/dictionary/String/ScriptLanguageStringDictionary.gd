...
# where keys are of type 'String' and values of type 'ScriptLanguage'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ScriptLanguageDictionary
...
func _assert_not_null_ScriptLanguage(value: ScriptLanguage):
    if (!value is ScriptLanguage):
        _crash("Was expecting type 'ScriptLanguage'")
