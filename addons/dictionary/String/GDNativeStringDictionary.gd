...
# where keys are of type 'String' and values of type 'GDNative'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GDNativeDictionary
...
func _assert_not_null_GDNative(value: GDNative):
    if (!value is GDNative):
        _crash("Was expecting type 'GDNative'")
