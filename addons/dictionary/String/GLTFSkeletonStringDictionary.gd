...
# where keys are of type 'String' and values of type 'GLTFSkeleton'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFSkeletonDictionary
...
func _assert_not_null_GLTFSkeleton(value: GLTFSkeleton):
    if (!value is GLTFSkeleton):
        _crash("Was expecting type 'GLTFSkeleton'")
