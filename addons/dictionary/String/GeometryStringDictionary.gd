...
# where keys are of type 'String' and values of type 'Geometry'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GeometryDictionary
...
func _assert_not_null_Geometry(value: Geometry):
    if (!value is Geometry):
        _crash("Was expecting type 'Geometry'")
