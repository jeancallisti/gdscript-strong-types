...
# where keys are of type 'String' and values of type 'ARVROrigin'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ARVROriginDictionary
...
func _assert_not_null_ARVROrigin(value: ARVROrigin):
    if (!value is ARVROrigin):
        _crash("Was expecting type 'ARVROrigin'")
