...
# where keys are of type 'String' and values of type 'TextFile'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TextFileDictionary
...
func _assert_not_null_TextFile(value: TextFile):
    if (!value is TextFile):
        _crash("Was expecting type 'TextFile'")
