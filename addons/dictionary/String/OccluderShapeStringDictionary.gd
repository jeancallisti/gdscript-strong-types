...
# where keys are of type 'String' and values of type 'OccluderShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name OccluderShapeDictionary
...
func _assert_not_null_OccluderShape(value: OccluderShape):
    if (!value is OccluderShape):
        _crash("Was expecting type 'OccluderShape'")
