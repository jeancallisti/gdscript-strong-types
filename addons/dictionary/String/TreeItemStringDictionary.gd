...
# where keys are of type 'String' and values of type 'TreeItem'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TreeItemDictionary
...
func _assert_not_null_TreeItem(value: TreeItem):
    if (!value is TreeItem):
        _crash("Was expecting type 'TreeItem'")
