...
# where keys are of type 'String' and values of type 'ARVRInterface'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ARVRInterfaceDictionary
...
func _assert_not_null_ARVRInterface(value: ARVRInterface):
    if (!value is ARVRInterface):
        _crash("Was expecting type 'ARVRInterface'")
