...
# where keys are of type 'String' and values of type 'InputMap'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name InputMapDictionary
...
func _assert_not_null_InputMap(value: InputMap):
    if (!value is InputMap):
        _crash("Was expecting type 'InputMap'")
