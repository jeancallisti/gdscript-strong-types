...
# where keys are of type 'String' and values of type 'ARVRServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ARVRServerDictionary
...
func _assert_not_null_ARVRServer(value: ARVRServer):
    if (!value is ARVRServer):
        _crash("Was expecting type 'ARVRServer'")
