...
# where keys are of type 'String' and values of type 'Navigation2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Navigation2DDictionary
...
func _assert_not_null_Navigation2D(value: Navigation2D):
    if (!value is Navigation2D):
        _crash("Was expecting type 'Navigation2D'")
