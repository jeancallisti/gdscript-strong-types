...
# where keys are of type 'String' and values of type 'AnimationPlayer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AnimationPlayerDictionary
...
func _assert_not_null_AnimationPlayer(value: AnimationPlayer):
    if (!value is AnimationPlayer):
        _crash("Was expecting type 'AnimationPlayer'")
