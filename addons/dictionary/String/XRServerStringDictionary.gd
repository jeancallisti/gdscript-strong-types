...
# where keys are of type 'String' and values of type 'XRServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name XRServerDictionary
...
func _assert_not_null_XRServer(value: XRServer):
    if (!value is XRServer):
        _crash("Was expecting type 'XRServer'")
