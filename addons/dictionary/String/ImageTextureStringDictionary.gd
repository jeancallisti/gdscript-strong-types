...
# where keys are of type 'String' and values of type 'ImageTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ImageTextureDictionary
...
func _assert_not_null_ImageTexture(value: ImageTexture):
    if (!value is ImageTexture):
        _crash("Was expecting type 'ImageTexture'")
