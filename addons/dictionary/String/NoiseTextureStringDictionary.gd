...
# where keys are of type 'String' and values of type 'NoiseTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NoiseTextureDictionary
...
func _assert_not_null_NoiseTexture(value: NoiseTexture):
    if (!value is NoiseTexture):
        _crash("Was expecting type 'NoiseTexture'")
