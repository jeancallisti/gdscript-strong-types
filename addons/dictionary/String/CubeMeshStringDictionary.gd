...
# where keys are of type 'String' and values of type 'CubeMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CubeMeshDictionary
...
func _assert_not_null_CubeMesh(value: CubeMesh):
    if (!value is CubeMesh):
        _crash("Was expecting type 'CubeMesh'")
