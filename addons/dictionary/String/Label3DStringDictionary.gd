...
# where keys are of type 'String' and values of type 'Label3D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Label3DDictionary
...
func _assert_not_null_Label3D(value: Label3D):
    if (!value is Label3D):
        _crash("Was expecting type 'Label3D'")
