...
# where keys are of type 'String' and values of type 'CSGPrimitive'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGPrimitiveDictionary
...
func _assert_not_null_CSGPrimitive(value: CSGPrimitive):
    if (!value is CSGPrimitive):
        _crash("Was expecting type 'CSGPrimitive'")
