...
# where keys are of type 'String' and values of type 'Area2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Area2DDictionary
...
func _assert_not_null_Area2D(value: Area2D):
    if (!value is Area2D):
        _crash("Was expecting type 'Area2D'")
