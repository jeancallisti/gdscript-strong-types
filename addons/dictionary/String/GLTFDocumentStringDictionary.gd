...
# where keys are of type 'String' and values of type 'GLTFDocument'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFDocumentDictionary
...
func _assert_not_null_GLTFDocument(value: GLTFDocument):
    if (!value is GLTFDocument):
        _crash("Was expecting type 'GLTFDocument'")
