...
# where keys are of type 'String' and values of type 'HMACContext'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HMACContextDictionary
...
func _assert_not_null_HMACContext(value: HMACContext):
    if (!value is HMACContext):
        _crash("Was expecting type 'HMACContext'")
