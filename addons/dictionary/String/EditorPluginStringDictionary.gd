...
# where keys are of type 'String' and values of type 'EditorPlugin'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name EditorPluginDictionary
...
func _assert_not_null_EditorPlugin(value: EditorPlugin):
    if (!value is EditorPlugin):
        _crash("Was expecting type 'EditorPlugin'")
