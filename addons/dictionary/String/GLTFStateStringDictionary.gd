...
# where keys are of type 'String' and values of type 'GLTFState'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFStateDictionary
...
func _assert_not_null_GLTFState(value: GLTFState):
    if (!value is GLTFState):
        _crash("Was expecting type 'GLTFState'")
