...
# where keys are of type 'String' and values of type 'CheckBox'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CheckBoxDictionary
...
func _assert_not_null_CheckBox(value: CheckBox):
    if (!value is CheckBox):
        _crash("Was expecting type 'CheckBox'")
