...
# where keys are of type 'String' and values of type 'ProximityGroup'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ProximityGroupDictionary
...
func _assert_not_null_ProximityGroup(value: ProximityGroup):
    if (!value is ProximityGroup):
        _crash("Was expecting type 'ProximityGroup'")
