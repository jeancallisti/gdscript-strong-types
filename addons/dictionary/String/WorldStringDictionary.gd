...
# where keys are of type 'String' and values of type 'World'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name WorldDictionary
...
func _assert_not_null_World(value: World):
    if (!value is World):
        _crash("Was expecting type 'World'")
