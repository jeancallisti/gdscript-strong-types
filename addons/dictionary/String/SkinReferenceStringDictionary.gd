...
# where keys are of type 'String' and values of type 'SkinReference'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SkinReferenceDictionary
...
func _assert_not_null_SkinReference(value: SkinReference):
    if (!value is SkinReference):
        _crash("Was expecting type 'SkinReference'")
