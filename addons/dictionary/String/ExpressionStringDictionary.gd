...
# where keys are of type 'String' and values of type 'Expression'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ExpressionDictionary
...
func _assert_not_null_Expression(value: Expression):
    if (!value is Expression):
        _crash("Was expecting type 'Expression'")
