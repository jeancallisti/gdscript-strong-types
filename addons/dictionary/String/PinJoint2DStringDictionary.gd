...
# where keys are of type 'String' and values of type 'PinJoint2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PinJoint2DDictionary
...
func _assert_not_null_PinJoint2D(value: PinJoint2D):
    if (!value is PinJoint2D):
        _crash("Was expecting type 'PinJoint2D'")
