...
# where keys are of type 'String' and values of type 'ScriptEditor'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ScriptEditorDictionary
...
func _assert_not_null_ScriptEditor(value: ScriptEditor):
    if (!value is ScriptEditor):
        _crash("Was expecting type 'ScriptEditor'")
