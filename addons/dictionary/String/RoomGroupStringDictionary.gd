...
# where keys are of type 'String' and values of type 'RoomGroup'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RoomGroupDictionary
...
func _assert_not_null_RoomGroup(value: RoomGroup):
    if (!value is RoomGroup):
        _crash("Was expecting type 'RoomGroup'")
