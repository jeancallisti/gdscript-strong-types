...
# where keys are of type 'String' and values of type 'Sprite'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpriteDictionary
...
func _assert_not_null_Sprite(value: Sprite):
    if (!value is Sprite):
        _crash("Was expecting type 'Sprite'")
