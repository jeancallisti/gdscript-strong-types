...
# where keys are of type 'String' and values of type 'VFlowContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name VFlowContainerDictionary
...
func _assert_not_null_VFlowContainer(value: VFlowContainer):
    if (!value is VFlowContainer):
        _crash("Was expecting type 'VFlowContainer'")
