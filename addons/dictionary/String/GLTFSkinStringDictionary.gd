...
# where keys are of type 'String' and values of type 'GLTFSkin'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFSkinDictionary
...
func _assert_not_null_GLTFSkin(value: GLTFSkin):
    if (!value is GLTFSkin):
        _crash("Was expecting type 'GLTFSkin'")
