...
# where keys are of type 'String' and values of type 'GLTFTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFTextureDictionary
...
func _assert_not_null_GLTFTexture(value: GLTFTexture):
    if (!value is GLTFTexture):
        _crash("Was expecting type 'GLTFTexture'")
