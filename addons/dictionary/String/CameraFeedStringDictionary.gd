...
# where keys are of type 'String' and values of type 'CameraFeed'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CameraFeedDictionary
...
func _assert_not_null_CameraFeed(value: CameraFeed):
    if (!value is CameraFeed):
        _crash("Was expecting type 'CameraFeed'")
