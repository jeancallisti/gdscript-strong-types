...
# where keys are of type 'String' and values of type 'ShaderMaterial'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ShaderMaterialDictionary
...
func _assert_not_null_ShaderMaterial(value: ShaderMaterial):
    if (!value is ShaderMaterial):
        _crash("Was expecting type 'ShaderMaterial'")
