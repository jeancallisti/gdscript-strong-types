...
# where keys are of type 'String' and values of type 'StyleBoxFlat'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StyleBoxFlatDictionary
...
func _assert_not_null_StyleBoxFlat(value: StyleBoxFlat):
    if (!value is StyleBoxFlat):
        _crash("Was expecting type 'StyleBoxFlat'")
