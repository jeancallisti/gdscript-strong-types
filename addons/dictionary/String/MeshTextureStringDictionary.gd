...
# where keys are of type 'String' and values of type 'MeshTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MeshTextureDictionary
...
func _assert_not_null_MeshTexture(value: MeshTexture):
    if (!value is MeshTexture):
        _crash("Was expecting type 'MeshTexture'")
