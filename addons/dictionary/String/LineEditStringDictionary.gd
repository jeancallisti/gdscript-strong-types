...
# where keys are of type 'String' and values of type 'LineEdit'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name LineEditDictionary
...
func _assert_not_null_LineEdit(value: LineEdit):
    if (!value is LineEdit):
        _crash("Was expecting type 'LineEdit'")
