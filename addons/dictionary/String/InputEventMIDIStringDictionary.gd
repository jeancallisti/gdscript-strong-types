...
# where keys are of type 'String' and values of type 'InputEventMIDI'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name InputEventMIDIDictionary
...
func _assert_not_null_InputEventMIDI(value: InputEventMIDI):
    if (!value is InputEventMIDI):
        _crash("Was expecting type 'InputEventMIDI'")
