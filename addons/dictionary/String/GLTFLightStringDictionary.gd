...
# where keys are of type 'String' and values of type 'GLTFLight'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFLightDictionary
...
func _assert_not_null_GLTFLight(value: GLTFLight):
    if (!value is GLTFLight):
        _crash("Was expecting type 'GLTFLight'")
