...
# where keys are of type 'String' and values of type 'Panel'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PanelDictionary
...
func _assert_not_null_Panel(value: Panel):
    if (!value is Panel):
        _crash("Was expecting type 'Panel'")
