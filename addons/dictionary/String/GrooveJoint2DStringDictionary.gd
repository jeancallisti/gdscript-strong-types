...
# where keys are of type 'String' and values of type 'GrooveJoint2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GrooveJoint2DDictionary
...
func _assert_not_null_GrooveJoint2D(value: GrooveJoint2D):
    if (!value is GrooveJoint2D):
        _crash("Was expecting type 'GrooveJoint2D'")
