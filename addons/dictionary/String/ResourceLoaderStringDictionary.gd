...
# where keys are of type 'String' and values of type 'ResourceLoader'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ResourceLoaderDictionary
...
func _assert_not_null_ResourceLoader(value: ResourceLoader):
    if (!value is ResourceLoader):
        _crash("Was expecting type 'ResourceLoader'")
