...
# where keys are of type 'String' and values of type 'SpotLight'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpotLightDictionary
...
func _assert_not_null_SpotLight(value: SpotLight):
    if (!value is SpotLight):
        _crash("Was expecting type 'SpotLight'")
