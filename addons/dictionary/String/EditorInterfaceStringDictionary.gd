...
# where keys are of type 'String' and values of type 'EditorInterface'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name EditorInterfaceDictionary
...
func _assert_not_null_EditorInterface(value: EditorInterface):
    if (!value is EditorInterface):
        _crash("Was expecting type 'EditorInterface'")
