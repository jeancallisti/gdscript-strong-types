...
# where keys are of type 'String' and values of type 'LineShape2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name LineShape2DDictionary
...
func _assert_not_null_LineShape2D(value: LineShape2D):
    if (!value is LineShape2D):
        _crash("Was expecting type 'LineShape2D'")
