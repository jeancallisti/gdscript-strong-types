...
# where keys are of type 'String' and values of type 'StaticBody2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StaticBody2DDictionary
...
func _assert_not_null_StaticBody2D(value: StaticBody2D):
    if (!value is StaticBody2D):
        _crash("Was expecting type 'StaticBody2D'")
