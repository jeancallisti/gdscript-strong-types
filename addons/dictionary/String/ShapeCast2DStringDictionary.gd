...
# where keys are of type 'String' and values of type 'ShapeCast2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ShapeCast2DDictionary
...
func _assert_not_null_ShapeCast2D(value: ShapeCast2D):
    if (!value is ShapeCast2D):
        _crash("Was expecting type 'ShapeCast2D'")
