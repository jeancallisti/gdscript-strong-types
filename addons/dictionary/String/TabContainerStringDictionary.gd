...
# where keys are of type 'String' and values of type 'TabContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TabContainerDictionary
...
func _assert_not_null_TabContainer(value: TabContainer):
    if (!value is TabContainer):
        _crash("Was expecting type 'TabContainer'")
