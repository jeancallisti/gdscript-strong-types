...
# where keys are of type 'String' and values of type 'StreamPeerSSL'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StreamPeerSSLDictionary
...
func _assert_not_null_StreamPeerSSL(value: StreamPeerSSL):
    if (!value is StreamPeerSSL):
        _crash("Was expecting type 'StreamPeerSSL'")
