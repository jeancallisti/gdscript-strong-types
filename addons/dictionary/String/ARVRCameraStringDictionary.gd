...
# where keys are of type 'String' and values of type 'ARVRCamera'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ARVRCameraDictionary
...
func _assert_not_null_ARVRCamera(value: ARVRCamera):
    if (!value is ARVRCamera):
        _crash("Was expecting type 'ARVRCamera'")
