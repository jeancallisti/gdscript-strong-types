...
# where keys are of type 'String' and values of type 'ConcavePolygonShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ConcavePolygonShapeDictionary
...
func _assert_not_null_ConcavePolygonShape(value: ConcavePolygonShape):
    if (!value is ConcavePolygonShape):
        _crash("Was expecting type 'ConcavePolygonShape'")
