...
# where keys are of type 'String' and values of type 'PanelContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PanelContainerDictionary
...
func _assert_not_null_PanelContainer(value: PanelContainer):
    if (!value is PanelContainer):
        _crash("Was expecting type 'PanelContainer'")
