...
# where keys are of type 'String' and values of type 'CSGMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGMeshDictionary
...
func _assert_not_null_CSGMesh(value: CSGMesh):
    if (!value is CSGMesh):
        _crash("Was expecting type 'CSGMesh'")
