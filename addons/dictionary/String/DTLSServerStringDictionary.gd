...
# where keys are of type 'String' and values of type 'DTLSServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name DTLSServerDictionary
...
func _assert_not_null_DTLSServer(value: DTLSServer):
    if (!value is DTLSServer):
        _crash("Was expecting type 'DTLSServer'")
