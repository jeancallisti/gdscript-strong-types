...
# where keys are of type 'String' and values of type 'CSGShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGShapeDictionary
...
func _assert_not_null_CSGShape(value: CSGShape):
    if (!value is CSGShape):
        _crash("Was expecting type 'CSGShape'")
