...
# where keys are of type 'String' and values of type 'GraphEdit'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GraphEditDictionary
...
func _assert_not_null_GraphEdit(value: GraphEdit):
    if (!value is GraphEdit):
        _crash("Was expecting type 'GraphEdit'")
