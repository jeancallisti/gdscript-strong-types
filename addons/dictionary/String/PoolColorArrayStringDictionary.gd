...
# where keys are of type 'String' and values of type 'PoolColorArray'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PoolColorArrayDictionary
...
func _assert_not_null_PoolColorArray(value: PoolColorArray):
    if (!value is PoolColorArray):
        _crash("Was expecting type 'PoolColorArray'")
