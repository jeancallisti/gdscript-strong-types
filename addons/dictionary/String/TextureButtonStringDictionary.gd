...
# where keys are of type 'String' and values of type 'TextureButton'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TextureButtonDictionary
...
func _assert_not_null_TextureButton(value: TextureButton):
    if (!value is TextureButton):
        _crash("Was expecting type 'TextureButton'")
