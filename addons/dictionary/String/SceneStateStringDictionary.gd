...
# where keys are of type 'String' and values of type 'SceneState'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SceneStateDictionary
...
func _assert_not_null_SceneState(value: SceneState):
    if (!value is SceneState):
        _crash("Was expecting type 'SceneState'")
