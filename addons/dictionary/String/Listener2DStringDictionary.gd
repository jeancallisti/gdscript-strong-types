...
# where keys are of type 'String' and values of type 'Listener2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Listener2DDictionary
...
func _assert_not_null_Listener2D(value: Listener2D):
    if (!value is Listener2D):
        _crash("Was expecting type 'Listener2D'")
