...
# where keys are of type 'String' and values of type 'PhysicsBody'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PhysicsBodyDictionary
...
func _assert_not_null_PhysicsBody(value: PhysicsBody):
    if (!value is PhysicsBody):
        _crash("Was expecting type 'PhysicsBody'")
