...
# where keys are of type 'String' and values of type 'PoolByteArray'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PoolByteArrayDictionary
...
func _assert_not_null_PoolByteArray(value: PoolByteArray):
    if (!value is PoolByteArray):
        _crash("Was expecting type 'PoolByteArray'")
