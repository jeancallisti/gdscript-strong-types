...
# where keys are of type 'String' and values of type 'StaticBody'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StaticBodyDictionary
...
func _assert_not_null_StaticBody(value: StaticBody):
    if (!value is StaticBody):
        _crash("Was expecting type 'StaticBody'")
