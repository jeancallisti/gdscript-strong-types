...
# where keys are of type 'String' and values of type 'CurveTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CurveTextureDictionary
...
func _assert_not_null_CurveTexture(value: CurveTexture):
    if (!value is CurveTexture):
        _crash("Was expecting type 'CurveTexture'")
