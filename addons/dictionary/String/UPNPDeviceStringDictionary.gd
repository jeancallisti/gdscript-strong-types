...
# where keys are of type 'String' and values of type 'UPNPDevice'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name UPNPDeviceDictionary
...
func _assert_not_null_UPNPDevice(value: UPNPDevice):
    if (!value is UPNPDevice):
        _crash("Was expecting type 'UPNPDevice'")
