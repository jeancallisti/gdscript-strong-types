...
# where keys are of type 'String' and values of type 'Position2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Position2DDictionary
...
func _assert_not_null_Position2D(value: Position2D):
    if (!value is Position2D):
        _crash("Was expecting type 'Position2D'")
