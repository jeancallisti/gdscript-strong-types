...
# where keys are of type 'String' and values of type 'GLTFAnimation'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFAnimationDictionary
...
func _assert_not_null_GLTFAnimation(value: GLTFAnimation):
    if (!value is GLTFAnimation):
        _crash("Was expecting type 'GLTFAnimation'")
