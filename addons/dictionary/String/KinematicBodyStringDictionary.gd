...
# where keys are of type 'String' and values of type 'KinematicBody'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name KinematicBodyDictionary
...
func _assert_not_null_KinematicBody(value: KinematicBody):
    if (!value is KinematicBody):
        _crash("Was expecting type 'KinematicBody'")
