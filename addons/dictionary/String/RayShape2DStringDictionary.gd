...
# where keys are of type 'String' and values of type 'RayShape2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RayShape2DDictionary
...
func _assert_not_null_RayShape2D(value: RayShape2D):
    if (!value is RayShape2D):
        _crash("Was expecting type 'RayShape2D'")
