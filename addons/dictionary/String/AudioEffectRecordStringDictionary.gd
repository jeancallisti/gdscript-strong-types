...
# where keys are of type 'String' and values of type 'AudioEffectRecord'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioEffectRecordDictionary
...
func _assert_not_null_AudioEffectRecord(value: AudioEffectRecord):
    if (!value is AudioEffectRecord):
        _crash("Was expecting type 'AudioEffectRecord'")
