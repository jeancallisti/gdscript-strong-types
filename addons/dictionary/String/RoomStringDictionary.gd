...
# where keys are of type 'String' and values of type 'Room'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RoomDictionary
...
func _assert_not_null_Room(value: Room):
    if (!value is Room):
        _crash("Was expecting type 'Room'")
