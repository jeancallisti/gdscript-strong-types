...
# where keys are of type 'String' and values of type 'GIProbe'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GIProbeDictionary
...
func _assert_not_null_GIProbe(value: GIProbe):
    if (!value is GIProbe):
        _crash("Was expecting type 'GIProbe'")
