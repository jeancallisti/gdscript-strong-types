...
# where keys are of type 'String' and values of type 'HTTPClient'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HTTPClientDictionary
...
func _assert_not_null_HTTPClient(value: HTTPClient):
    if (!value is HTTPClient):
        _crash("Was expecting type 'HTTPClient'")
