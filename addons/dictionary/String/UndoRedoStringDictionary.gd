...
# where keys are of type 'String' and values of type 'UndoRedo'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name UndoRedoDictionary
...
func _assert_not_null_UndoRedo(value: UndoRedo):
    if (!value is UndoRedo):
        _crash("Was expecting type 'UndoRedo'")
