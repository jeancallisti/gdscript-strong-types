...
# where keys are of type 'String' and values of type 'Popup'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PopupDictionary
...
func _assert_not_null_Popup(value: Popup):
    if (!value is Popup):
        _crash("Was expecting type 'Popup'")
