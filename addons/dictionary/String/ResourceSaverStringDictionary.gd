...
# where keys are of type 'String' and values of type 'ResourceSaver'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ResourceSaverDictionary
...
func _assert_not_null_ResourceSaver(value: ResourceSaver):
    if (!value is ResourceSaver):
        _crash("Was expecting type 'ResourceSaver'")
