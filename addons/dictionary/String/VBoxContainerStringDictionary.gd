...
# where keys are of type 'String' and values of type 'VBoxContainer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name VBoxContainerDictionary
...
func _assert_not_null_VBoxContainer(value: VBoxContainer):
    if (!value is VBoxContainer):
        _crash("Was expecting type 'VBoxContainer'")
