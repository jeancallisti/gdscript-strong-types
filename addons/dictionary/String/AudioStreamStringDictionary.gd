...
# where keys are of type 'String' and values of type 'AudioStream'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioStreamDictionary
...
func _assert_not_null_AudioStream(value: AudioStream):
    if (!value is AudioStream):
        _crash("Was expecting type 'AudioStream'")
