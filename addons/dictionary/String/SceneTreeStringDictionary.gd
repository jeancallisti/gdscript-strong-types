...
# where keys are of type 'String' and values of type 'SceneTree'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SceneTreeDictionary
...
func _assert_not_null_SceneTree(value: SceneTree):
    if (!value is SceneTree):
        _crash("Was expecting type 'SceneTree'")
