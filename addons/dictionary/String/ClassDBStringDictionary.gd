...
# where keys are of type 'String' and values of type 'ClassDB'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ClassDBDictionary
...
func _assert_not_null_ClassDB(value: ClassDB):
    if (!value is ClassDB):
        _crash("Was expecting type 'ClassDB'")
