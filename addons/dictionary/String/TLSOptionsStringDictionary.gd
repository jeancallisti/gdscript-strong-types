...
# where keys are of type 'String' and values of type 'TLSOptions'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TLSOptionsDictionary
...
func set(key: String, value: TLSOptions):
...
func set_nullable(key: String, value: TLSOptions):
    _assert_not_null(key)
    _assert_String(key)
    _assert_TLSOptions(value)
    d[key] = value

func get(key: String) -> TLSOptions:
    _assert_not_null(key)
    _assert_String(key)
    return d[key]	
