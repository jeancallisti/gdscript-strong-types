...
# where keys are of type 'String' and values of type 'JNISingleton'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name JNISingletonDictionary
...
func _assert_not_null_JNISingleton(value: JNISingleton):
    if (!value is JNISingleton):
        _crash("Was expecting type 'JNISingleton'")
