...
# where keys are of type 'String' and values of type 'BackBufferCopy'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name BackBufferCopyDictionary
...
func _assert_not_null_BackBufferCopy(value: BackBufferCopy):
    if (!value is BackBufferCopy):
        _crash("Was expecting type 'BackBufferCopy'")
