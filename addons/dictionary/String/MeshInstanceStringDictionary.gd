...
# where keys are of type 'String' and values of type 'MeshInstance'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MeshInstanceDictionary
...
func _assert_not_null_MeshInstance(value: MeshInstance):
    if (!value is MeshInstance):
        _crash("Was expecting type 'MeshInstance'")
