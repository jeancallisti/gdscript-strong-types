...
# where keys are of type 'String' and values of type 'FileDialog'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name FileDialogDictionary
...
func _assert_not_null_FileDialog(value: FileDialog):
    if (!value is FileDialog):
        _crash("Was expecting type 'FileDialog'")
