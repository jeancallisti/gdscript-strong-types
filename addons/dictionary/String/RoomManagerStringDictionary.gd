...
# where keys are of type 'String' and values of type 'RoomManager'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RoomManagerDictionary
...
func _assert_not_null_RoomManager(value: RoomManager):
    if (!value is RoomManager):
        _crash("Was expecting type 'RoomManager'")
