...
# where keys are of type 'String' and values of type 'EncodedObjectAsID'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name EncodedObjectAsIDDictionary
...
func _assert_not_null_EncodedObjectAsID(value: EncodedObjectAsID):
    if (!value is EncodedObjectAsID):
        _crash("Was expecting type 'EncodedObjectAsID'")
