...
# where keys are of type 'String' and values of type 'Node2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Node2DDictionary
...
func _assert_not_null_Node2D(value: Node2D):
    if (!value is Node2D):
        _crash("Was expecting type 'Node2D'")
