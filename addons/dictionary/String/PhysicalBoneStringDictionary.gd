...
# where keys are of type 'String' and values of type 'PhysicalBone'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PhysicalBoneDictionary
...
func _assert_not_null_PhysicalBone(value: PhysicalBone):
    if (!value is PhysicalBone):
        _crash("Was expecting type 'PhysicalBone'")
