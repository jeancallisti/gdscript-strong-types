...
# where keys are of type 'String' and values of type 'TileMap'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TileMapDictionary
...
func _assert_not_null_TileMap(value: TileMap):
    if (!value is TileMap):
        _crash("Was expecting type 'TileMap'")
