...
# where keys are of type 'String' and values of type 'CameraServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CameraServerDictionary
...
func _assert_not_null_CameraServer(value: CameraServer):
    if (!value is CameraServer):
        _crash("Was expecting type 'CameraServer'")
