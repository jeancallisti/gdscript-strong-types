...
# where keys are of type 'String' and values of type 'VSlider'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name VSliderDictionary
...
func _assert_not_null_VSlider(value: VSlider):
    if (!value is VSlider):
        _crash("Was expecting type 'VSlider'")
