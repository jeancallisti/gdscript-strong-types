...
# where keys are of type 'String' and values of type 'MultiMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MultiMeshDictionary
...
func _assert_not_null_MultiMesh(value: MultiMesh):
    if (!value is MultiMesh):
        _crash("Was expecting type 'MultiMesh'")
