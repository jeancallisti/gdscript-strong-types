...
# where keys are of type 'String' and values of type 'Light'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name LightDictionary
...
func _assert_not_null_Light(value: Light):
    if (!value is Light):
        _crash("Was expecting type 'Light'")
