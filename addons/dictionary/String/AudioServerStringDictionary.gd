...
# where keys are of type 'String' and values of type 'AudioServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioServerDictionary
...
func _assert_not_null_AudioServer(value: AudioServer):
    if (!value is AudioServer):
        _crash("Was expecting type 'AudioServer'")
