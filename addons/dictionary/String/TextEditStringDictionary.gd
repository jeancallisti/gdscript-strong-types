...
# where keys are of type 'String' and values of type 'TextEdit'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TextEditDictionary
...
func _assert_not_null_TextEdit(value: TextEdit):
    if (!value is TextEdit):
        _crash("Was expecting type 'TextEdit'")
