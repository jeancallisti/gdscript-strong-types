...
# where keys are of type 'String' and values of type 'GradientTexture2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GradientTexture2DDictionary
...
func _assert_not_null_GradientTexture2D(value: GradientTexture2D):
    if (!value is GradientTexture2D):
        _crash("Was expecting type 'GradientTexture2D'")
