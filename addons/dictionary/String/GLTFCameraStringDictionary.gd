...
# where keys are of type 'String' and values of type 'GLTFCamera'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFCameraDictionary
...
func _assert_not_null_GLTFCamera(value: GLTFCamera):
    if (!value is GLTFCamera):
        _crash("Was expecting type 'GLTFCamera'")
