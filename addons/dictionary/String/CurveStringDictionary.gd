...
# where keys are of type 'String' and values of type 'Curve'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CurveDictionary
...
func _assert_not_null_Curve(value: Curve):
    if (!value is Curve):
        _crash("Was expecting type 'Curve'")
