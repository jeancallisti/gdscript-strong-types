...
# where keys are of type 'String' and values of type 'UPNP'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name UPNPDictionary
...
func _assert_not_null_UPNP(value: UPNP):
    if (!value is UPNP):
        _crash("Was expecting type 'UPNP'")
