...
# where keys are of type 'String' and values of type 'CapsuleShape2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CapsuleShape2DDictionary
...
func _assert_not_null_CapsuleShape2D(value: CapsuleShape2D):
    if (!value is CapsuleShape2D):
        _crash("Was expecting type 'CapsuleShape2D'")
