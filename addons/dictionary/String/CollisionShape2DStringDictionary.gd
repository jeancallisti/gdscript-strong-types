...
# where keys are of type 'String' and values of type 'CollisionShape2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CollisionShape2DDictionary
...
func _assert_not_null_CollisionShape2D(value: CollisionShape2D):
    if (!value is CollisionShape2D):
        _crash("Was expecting type 'CollisionShape2D'")
