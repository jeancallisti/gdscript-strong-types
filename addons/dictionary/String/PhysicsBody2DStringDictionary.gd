...
# where keys are of type 'String' and values of type 'PhysicsBody2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PhysicsBody2DDictionary
...
func _assert_not_null_PhysicsBody2D(value: PhysicsBody2D):
    if (!value is PhysicsBody2D):
        _crash("Was expecting type 'PhysicsBody2D'")
