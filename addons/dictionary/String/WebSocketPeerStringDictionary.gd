...
# where keys are of type 'String' and values of type 'WebSocketPeer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name WebSocketPeerDictionary
...
func _assert_not_null_WebSocketPeer(value: WebSocketPeer):
    if (!value is WebSocketPeer):
        _crash("Was expecting type 'WebSocketPeer'")
