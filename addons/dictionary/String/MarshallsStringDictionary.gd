...
# where keys are of type 'String' and values of type 'Marshalls'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MarshallsDictionary
...
func _assert_not_null_Marshalls(value: Marshalls):
    if (!value is Marshalls):
        _crash("Was expecting type 'Marshalls'")
