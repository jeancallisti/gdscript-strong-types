...
# where keys are of type 'String' and values of type 'CSGTorus'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGTorusDictionary
...
func _assert_not_null_CSGTorus(value: CSGTorus):
    if (!value is CSGTorus):
        _crash("Was expecting type 'CSGTorus'")
