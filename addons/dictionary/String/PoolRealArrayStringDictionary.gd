...
# where keys are of type 'String' and values of type 'PoolRealArray'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PoolRealArrayDictionary
...
func _assert_not_null_PoolRealArray(value: PoolRealArray):
    if (!value is PoolRealArray):
        _crash("Was expecting type 'PoolRealArray'")
