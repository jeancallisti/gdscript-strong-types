...
# where keys are of type 'String' and values of type 'Position3D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Position3DDictionary
...
func _assert_not_null_Position3D(value: Position3D):
    if (!value is Position3D):
        _crash("Was expecting type 'Position3D'")
