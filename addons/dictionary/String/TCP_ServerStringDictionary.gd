...
# where keys are of type 'String' and values of type 'TCP_Server'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TCP_ServerDictionary
...
func _assert_not_null_TCP_Server(value: TCP_Server):
    if (!value is TCP_Server):
        _crash("Was expecting type 'TCP_Server'")
