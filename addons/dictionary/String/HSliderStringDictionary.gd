...
# where keys are of type 'String' and values of type 'HSlider'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HSliderDictionary
...
func _assert_not_null_HSlider(value: HSlider):
    if (!value is HSlider):
        _crash("Was expecting type 'HSlider'")
