...
# where keys are of type 'String' and values of type 'MeshInstance2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MeshInstance2DDictionary
...
func _assert_not_null_MeshInstance2D(value: MeshInstance2D):
    if (!value is MeshInstance2D):
        _crash("Was expecting type 'MeshInstance2D'")
