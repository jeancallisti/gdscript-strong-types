...
# where keys are of type 'String' and values of type 'SpriteFrames'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpriteFramesDictionary
...
func _assert_not_null_SpriteFrames(value: SpriteFrames):
    if (!value is SpriteFrames):
        _crash("Was expecting type 'SpriteFrames'")
