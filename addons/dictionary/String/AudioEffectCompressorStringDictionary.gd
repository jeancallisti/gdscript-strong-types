...
# where keys are of type 'String' and values of type 'AudioEffectCompressor'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioEffectCompressorDictionary
...
func _assert_not_null_AudioEffectCompressor(value: AudioEffectCompressor):
    if (!value is AudioEffectCompressor):
        _crash("Was expecting type 'AudioEffectCompressor'")
