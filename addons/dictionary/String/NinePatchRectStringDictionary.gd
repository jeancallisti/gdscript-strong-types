...
# where keys are of type 'String' and values of type 'NinePatchRect'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NinePatchRectDictionary
...
func _assert_not_null_NinePatchRect(value: NinePatchRect):
    if (!value is NinePatchRect):
        _crash("Was expecting type 'NinePatchRect'")
