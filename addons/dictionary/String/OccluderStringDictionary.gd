...
# where keys are of type 'String' and values of type 'Occluder'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name OccluderDictionary
...
func _assert_not_null_Occluder(value: Occluder):
    if (!value is Occluder):
        _crash("Was expecting type 'Occluder'")
