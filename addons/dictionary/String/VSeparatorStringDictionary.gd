...
# where keys are of type 'String' and values of type 'VSeparator'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name VSeparatorDictionary
...
func _assert_not_null_VSeparator(value: VSeparator):
    if (!value is VSeparator):
        _crash("Was expecting type 'VSeparator'")
