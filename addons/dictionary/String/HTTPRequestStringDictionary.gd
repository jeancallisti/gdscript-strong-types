...
# where keys are of type 'String' and values of type 'HTTPRequest'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name HTTPRequestDictionary
...
func _assert_not_null_HTTPRequest(value: HTTPRequest):
    if (!value is HTTPRequest):
        _crash("Was expecting type 'HTTPRequest'")
