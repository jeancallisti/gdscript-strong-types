...
# where keys are of type 'String' and values of type 'LargeTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name LargeTextureDictionary
...
func _assert_not_null_LargeTexture(value: LargeTexture):
    if (!value is LargeTexture):
        _crash("Was expecting type 'LargeTexture'")
