...
# where keys are of type 'String' and values of type 'CullInstance'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CullInstanceDictionary
...
func _assert_not_null_CullInstance(value: CullInstance):
    if (!value is CullInstance):
        _crash("Was expecting type 'CullInstance'")
