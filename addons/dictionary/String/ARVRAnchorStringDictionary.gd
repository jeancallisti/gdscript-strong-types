...
# where keys are of type 'String' and values of type 'ARVRAnchor'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ARVRAnchorDictionary
...
func _assert_not_null_ARVRAnchor(value: ARVRAnchor):
    if (!value is ARVRAnchor):
        _crash("Was expecting type 'ARVRAnchor'")
