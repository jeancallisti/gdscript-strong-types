...
# where keys are of type 'String' and values of type 'CSGPolygon'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGPolygonDictionary
...
func _assert_not_null_CSGPolygon(value: CSGPolygon):
    if (!value is CSGPolygon):
        _crash("Was expecting type 'CSGPolygon'")
