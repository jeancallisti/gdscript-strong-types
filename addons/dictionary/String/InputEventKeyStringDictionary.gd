...
# where keys are of type 'String' and values of type 'InputEventKey'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name InputEventKeyDictionary
...
func _assert_not_null_InputEventKey(value: InputEventKey):
    if (!value is InputEventKey):
        _crash("Was expecting type 'InputEventKey'")
