...
# where keys are of type 'String' and values of type 'Control'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ControlDictionary
...
func _assert_not_null_Control(value: Control):
    if (!value is Control):
        _crash("Was expecting type 'Control'")
