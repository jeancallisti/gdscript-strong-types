...
# where keys are of type 'String' and values of type 'Particles2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Particles2DDictionary
...
func _assert_not_null_Particles2D(value: Particles2D):
    if (!value is Particles2D):
        _crash("Was expecting type 'Particles2D'")
