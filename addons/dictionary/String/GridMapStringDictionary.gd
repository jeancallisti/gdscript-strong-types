...
# where keys are of type 'String' and values of type 'GridMap'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GridMapDictionary
...
func _assert_not_null_GridMap(value: GridMap):
    if (!value is GridMap):
        _crash("Was expecting type 'GridMap'")
