...
# where keys are of type 'String' and values of type 'TextMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TextMeshDictionary
...
func _assert_not_null_TextMesh(value: TextMesh):
    if (!value is TextMesh):
        _crash("Was expecting type 'TextMesh'")
