...
# where keys are of type 'String' and values of type 'TextureRect'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name TextureRectDictionary
...
func _assert_not_null_TextureRect(value: TextureRect):
    if (!value is TextureRect):
        _crash("Was expecting type 'TextureRect'")
