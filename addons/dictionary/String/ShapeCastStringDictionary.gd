...
# where keys are of type 'String' and values of type 'ShapeCast'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ShapeCastDictionary
...
func _assert_not_null_ShapeCast(value: ShapeCast):
    if (!value is ShapeCast):
        _crash("Was expecting type 'ShapeCast'")
