...
# where keys are of type 'String' and values of type 'CubeMap'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CubeMapDictionary
...
func _assert_not_null_CubeMap(value: CubeMap):
    if (!value is CubeMap):
        _crash("Was expecting type 'CubeMap'")
