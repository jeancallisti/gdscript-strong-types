...
# where keys are of type 'String' and values of type 'RichTextEffect'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RichTextEffectDictionary
...
func _assert_not_null_RichTextEffect(value: RichTextEffect):
    if (!value is RichTextEffect):
        _crash("Was expecting type 'RichTextEffect'")
