...
# where keys are of type 'String' and values of type 'RayCast'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RayCastDictionary
...
func _assert_not_null_RayCast(value: RayCast):
    if (!value is RayCast):
        _crash("Was expecting type 'RayCast'")
