...
# where keys are of type 'String' and values of type 'AudioEffectLowShelfFilter'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioEffectLowShelfFilterDictionary
...
func _assert_not_null_AudioEffectLowShelfFilter(value: AudioEffectLowShelfFilter):
    if (!value is AudioEffectLowShelfFilter):
        _crash("Was expecting type 'AudioEffectLowShelfFilter'")
