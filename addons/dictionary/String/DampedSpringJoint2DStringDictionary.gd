...
# where keys are of type 'String' and values of type 'DampedSpringJoint2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name DampedSpringJoint2DDictionary
...
func _assert_not_null_DampedSpringJoint2D(value: DampedSpringJoint2D):
    if (!value is DampedSpringJoint2D):
        _crash("Was expecting type 'DampedSpringJoint2D'")
