...
# where keys are of type 'String' and values of type 'SoftBody'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SoftBodyDictionary
...
func _assert_not_null_SoftBody(value: SoftBody):
    if (!value is SoftBody):
        _crash("Was expecting type 'SoftBody'")
