...
# where keys are of type 'String' and values of type 'PointMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PointMeshDictionary
...
func _assert_not_null_PointMesh(value: PointMesh):
    if (!value is PointMesh):
        _crash("Was expecting type 'PointMesh'")
