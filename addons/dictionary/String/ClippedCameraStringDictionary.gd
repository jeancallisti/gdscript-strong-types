...
# where keys are of type 'String' and values of type 'ClippedCamera'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ClippedCameraDictionary
...
func _assert_not_null_ClippedCamera(value: ClippedCamera):
    if (!value is ClippedCamera):
        _crash("Was expecting type 'ClippedCamera'")
