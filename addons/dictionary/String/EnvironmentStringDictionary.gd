...
# where keys are of type 'String' and values of type 'Environment'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name EnvironmentDictionary
...
func _assert_not_null_Environment(value: Environment):
    if (!value is Environment):
        _crash("Was expecting type 'Environment'")
