...
# where keys are of type 'String' and values of type 'Button'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ButtonDictionary
...
func _assert_not_null_Button(value: Button):
    if (!value is Button):
        _crash("Was expecting type 'Button'")
