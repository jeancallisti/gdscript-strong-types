...
# where keys are of type 'String' and values of type 'RayShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RayShapeDictionary
...
func _assert_not_null_RayShape(value: RayShape):
    if (!value is RayShape):
        _crash("Was expecting type 'RayShape'")
