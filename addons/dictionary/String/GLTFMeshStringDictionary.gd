...
# where keys are of type 'String' and values of type 'GLTFMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFMeshDictionary
...
func _assert_not_null_GLTFMesh(value: GLTFMesh):
    if (!value is GLTFMesh):
        _crash("Was expecting type 'GLTFMesh'")
