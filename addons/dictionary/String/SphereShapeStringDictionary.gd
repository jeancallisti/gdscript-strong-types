...
# where keys are of type 'String' and values of type 'SphereShape'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SphereShapeDictionary
...
func _assert_not_null_SphereShape(value: SphereShape):
    if (!value is SphereShape):
        _crash("Was expecting type 'SphereShape'")
