...
# where keys are of type 'String' and values of type 'GLTFAccessor'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFAccessorDictionary
...
func _assert_not_null_GLTFAccessor(value: GLTFAccessor):
    if (!value is GLTFAccessor):
        _crash("Was expecting type 'GLTFAccessor'")
