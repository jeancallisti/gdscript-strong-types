...
# where keys are of type 'String' and values of type 'Area'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AreaDictionary
...
func _assert_not_null_Area(value: Area):
    if (!value is Area):
        _crash("Was expecting type 'Area'")
