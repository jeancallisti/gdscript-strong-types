...
# where keys are of type 'String' and values of type 'CSGBox'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name CSGBoxDictionary
...
func _assert_not_null_CSGBox(value: CSGBox):
    if (!value is CSGBox):
        _crash("Was expecting type 'CSGBox'")
