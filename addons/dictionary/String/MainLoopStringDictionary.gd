...
# where keys are of type 'String' and values of type 'MainLoop'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name MainLoopDictionary
...
func _assert_not_null_MainLoop(value: MainLoop):
    if (!value is MainLoop):
        _crash("Was expecting type 'MainLoop'")
