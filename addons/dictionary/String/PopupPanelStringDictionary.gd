...
# where keys are of type 'String' and values of type 'PopupPanel'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PopupPanelDictionary
...
func _assert_not_null_PopupPanel(value: PopupPanel):
    if (!value is PopupPanel):
        _crash("Was expecting type 'PopupPanel'")
