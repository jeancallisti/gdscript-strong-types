...
# where keys are of type 'String' and values of type 'PathFollow2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PathFollow2DDictionary
...
func _assert_not_null_PathFollow2D(value: PathFollow2D):
    if (!value is PathFollow2D):
        _crash("Was expecting type 'PathFollow2D'")
