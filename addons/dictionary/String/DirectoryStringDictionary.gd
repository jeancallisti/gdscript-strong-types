...
# where keys are of type 'String' and values of type 'Directory'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name DirectoryDictionary
...
func _assert_not_null_Directory(value: Directory):
    if (!value is Directory):
        _crash("Was expecting type 'Directory'")
