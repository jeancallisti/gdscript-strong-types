...
# where keys are of type 'String' and values of type 'WebXRInterface'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name WebXRInterfaceDictionary
...
func _assert_not_null_WebXRInterface(value: WebXRInterface):
    if (!value is WebXRInterface):
        _crash("Was expecting type 'WebXRInterface'")
