...
# where keys are of type 'String' and values of type 'JavaClassWrapper'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name JavaClassWrapperDictionary
...
func _assert_not_null_JavaClassWrapper(value: JavaClassWrapper):
    if (!value is JavaClassWrapper):
        _crash("Was expecting type 'JavaClassWrapper'")
