...
# where keys are of type 'String' and values of type 'Geometry2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Geometry2DDictionary
...
func _assert_not_null_Geometry2D(value: Geometry2D):
    if (!value is Geometry2D):
        _crash("Was expecting type 'Geometry2D'")
