...
# where keys are of type 'String' and values of type 'ArrayMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ArrayMeshDictionary
...
func _assert_not_null_ArrayMesh(value: ArrayMesh):
    if (!value is ArrayMesh):
        _crash("Was expecting type 'ArrayMesh'")
