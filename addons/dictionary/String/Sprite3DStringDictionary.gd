...
# where keys are of type 'String' and values of type 'Sprite3D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Sprite3DDictionary
...
func _assert_not_null_Sprite3D(value: Sprite3D):
    if (!value is Sprite3D):
        _crash("Was expecting type 'Sprite3D'")
