...
# where keys are of type 'String' and values of type 'PoolStringArray'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PoolStringArrayDictionary
...
func _assert_not_null_PoolStringArray(value: PoolStringArray):
    if (!value is PoolStringArray):
        _crash("Was expecting type 'PoolStringArray'")
