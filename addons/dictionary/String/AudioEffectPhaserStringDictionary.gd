...
# where keys are of type 'String' and values of type 'AudioEffectPhaser'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioEffectPhaserDictionary
...
func _assert_not_null_AudioEffectPhaser(value: AudioEffectPhaser):
    if (!value is AudioEffectPhaser):
        _crash("Was expecting type 'AudioEffectPhaser'")
