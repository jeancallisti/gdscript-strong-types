...
# where keys are of type 'String' and values of type 'SpatialGizmo'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SpatialGizmoDictionary
...
func _assert_not_null_SpatialGizmo(value: SpatialGizmo):
    if (!value is SpatialGizmo):
        _crash("Was expecting type 'SpatialGizmo'")
