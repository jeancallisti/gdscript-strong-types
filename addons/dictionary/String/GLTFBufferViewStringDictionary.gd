...
# where keys are of type 'String' and values of type 'GLTFBufferView'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name GLTFBufferViewDictionary
...
func _assert_not_null_GLTFBufferView(value: GLTFBufferView):
    if (!value is GLTFBufferView):
        _crash("Was expecting type 'GLTFBufferView'")
