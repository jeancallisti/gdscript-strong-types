...
# where keys are of type 'String' and values of type 'ButtonGroup'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ButtonGroupDictionary
...
func _assert_not_null_ButtonGroup(value: ButtonGroup):
    if (!value is ButtonGroup):
        _crash("Was expecting type 'ButtonGroup'")
