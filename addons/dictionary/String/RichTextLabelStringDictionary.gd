...
# where keys are of type 'String' and values of type 'RichTextLabel'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RichTextLabelDictionary
...
func _assert_not_null_RichTextLabel(value: RichTextLabel):
    if (!value is RichTextLabel):
        _crash("Was expecting type 'RichTextLabel'")
