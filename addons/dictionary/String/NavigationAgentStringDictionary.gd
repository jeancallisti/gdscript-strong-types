...
# where keys are of type 'String' and values of type 'NavigationAgent'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NavigationAgentDictionary
...
func _assert_not_null_NavigationAgent(value: NavigationAgent):
    if (!value is NavigationAgent):
        _crash("Was expecting type 'NavigationAgent'")
