...
# where keys are of type 'String' and values of type 'Transform2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Transform2DDictionary
...
func _assert_not_null_Transform2D(value: Transform2D):
    if (!value is Transform2D):
        _crash("Was expecting type 'Transform2D'")
