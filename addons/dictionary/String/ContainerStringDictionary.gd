...
# where keys are of type 'String' and values of type 'Container'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ContainerDictionary
...
func _assert_not_null_Container(value: Container):
    if (!value is Container):
        _crash("Was expecting type 'Container'")
