...
# where keys are of type 'String' and values of type 'RenderingServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RenderingServerDictionary
...
func _assert_not_null_RenderingServer(value: RenderingServer):
    if (!value is RenderingServer):
        _crash("Was expecting type 'RenderingServer'")
