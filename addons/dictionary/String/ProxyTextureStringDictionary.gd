...
# where keys are of type 'String' and values of type 'ProxyTexture'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ProxyTextureDictionary
...
func _assert_not_null_ProxyTexture(value: ProxyTexture):
    if (!value is ProxyTexture):
        _crash("Was expecting type 'ProxyTexture'")
