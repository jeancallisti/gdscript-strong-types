...
# where keys are of type 'String' and values of type 'DisplayServer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name DisplayServerDictionary
...
func _assert_not_null_DisplayServer(value: DisplayServer):
    if (!value is DisplayServer):
        _crash("Was expecting type 'DisplayServer'")
