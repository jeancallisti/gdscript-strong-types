...
# where keys are of type 'String' and values of type 'EngineProfiler'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name EngineProfilerDictionary
...
func _assert_not_null_EngineProfiler(value: EngineProfiler):
    if (!value is EngineProfiler):
        _crash("Was expecting type 'EngineProfiler'")
