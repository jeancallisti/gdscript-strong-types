...
# where keys are of type 'String' and values of type 'PacketPeerUDP'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PacketPeerUDPDictionary
...
func set(key: String, value: PacketPeerUDP):
...
func set_nullable(key: String, value: PacketPeerUDP):
    _assert_not_null(key)
    _assert_String(key)
    _assert_PacketPeerUDP(value)
    d[key] = value

func get(key: String) -> PacketPeerUDP:
    _assert_not_null(key)
    _assert_String(key)
    return d[key]	
