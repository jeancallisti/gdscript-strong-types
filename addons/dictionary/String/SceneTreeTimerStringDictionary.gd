...
# where keys are of type 'String' and values of type 'SceneTreeTimer'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SceneTreeTimerDictionary
...
func _assert_not_null_SceneTreeTimer(value: SceneTreeTimer):
    if (!value is SceneTreeTimer):
        _crash("Was expecting type 'SceneTreeTimer'")
