...
# where keys are of type 'String' and values of type 'StyleBoxEmpty'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name StyleBoxEmptyDictionary
...
func _assert_not_null_StyleBoxEmpty(value: StyleBoxEmpty):
    if (!value is StyleBoxEmpty):
        _crash("Was expecting type 'StyleBoxEmpty'")
