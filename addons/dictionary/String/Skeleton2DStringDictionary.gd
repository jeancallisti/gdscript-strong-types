...
# where keys are of type 'String' and values of type 'Skeleton2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Skeleton2DDictionary
...
func _assert_not_null_Skeleton2D(value: Skeleton2D):
    if (!value is Skeleton2D):
        _crash("Was expecting type 'Skeleton2D'")
