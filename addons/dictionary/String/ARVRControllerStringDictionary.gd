...
# where keys are of type 'String' and values of type 'ARVRController'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ARVRControllerDictionary
...
func _assert_not_null_ARVRController(value: ARVRController):
    if (!value is ARVRController):
        _crash("Was expecting type 'ARVRController'")
