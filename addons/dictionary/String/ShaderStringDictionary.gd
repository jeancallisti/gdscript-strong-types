...
# where keys are of type 'String' and values of type 'Shader'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name ShaderDictionary
...
func _assert_not_null_Shader(value: Shader):
    if (!value is Shader):
        _crash("Was expecting type 'Shader'")
