...
# where keys are of type 'String' and values of type 'Texture3D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name Texture3DDictionary
...
func _assert_not_null_Texture3D(value: Texture3D):
    if (!value is Texture3D):
        _crash("Was expecting type 'Texture3D'")
