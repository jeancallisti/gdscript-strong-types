...
# where keys are of type 'String' and values of type 'Portal'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PortalDictionary
...
func _assert_not_null_Portal(value: Portal):
    if (!value is Portal):
        _crash("Was expecting type 'Portal'")
