...
# where keys are of type 'String' and values of type 'NavigationObstacle2D'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name NavigationObstacle2DDictionary
...
func _assert_not_null_NavigationObstacle2D(value: NavigationObstacle2D):
    if (!value is NavigationObstacle2D):
        _crash("Was expecting type 'NavigationObstacle2D'")
