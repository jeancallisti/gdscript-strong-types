...
# where keys are of type 'String' and values of type 'char'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name charDictionary
...
func _assert_not_null_char(value: char):
    if (!value is char):
        _crash("Was expecting type 'char'")
