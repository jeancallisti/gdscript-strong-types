...
# where keys are of type 'String' and values of type 'PinJoint'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name PinJointDictionary
...
func _assert_not_null_PinJoint(value: PinJoint):
    if (!value is PinJoint):
        _crash("Was expecting type 'PinJoint'")
