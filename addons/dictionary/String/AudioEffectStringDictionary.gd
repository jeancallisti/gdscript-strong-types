...
# where keys are of type 'String' and values of type 'AudioEffect'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name AudioEffectDictionary
...
func _assert_not_null_AudioEffect(value: AudioEffect):
    if (!value is AudioEffect):
        _crash("Was expecting type 'AudioEffect'")
