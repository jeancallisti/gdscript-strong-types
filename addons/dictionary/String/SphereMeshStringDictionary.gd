...
# where keys are of type 'String' and values of type 'SphereMesh'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name SphereMeshDictionary
...
func _assert_not_null_SphereMesh(value: SphereMesh):
    if (!value is SphereMesh):
        _crash("Was expecting type 'SphereMesh'")
