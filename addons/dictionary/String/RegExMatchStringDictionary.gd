...
# where keys are of type 'String' and values of type 'RegExMatch'.
# It is not meant to be attached to a Node (unless you want to!)
# You can generate more classes like this one for your own types.
class_name RegExMatchDictionary
...
func _assert_not_null_RegExMatch(value: RegExMatch):
    if (!value is RegExMatch):
        _crash("Was expecting type 'RegExMatch'")
